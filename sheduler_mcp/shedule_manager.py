...
def meeting_message(
    ...
    if not free_slots:
        return (
            "Write a polite group message informing that "
            "no common free time is available for all members today."
        )

    slots = ", ".join(free_slots)

    return (
        "Write a friendly and professional group message saying that "
        f"all members ({', '.join(members)}) are available at the following times: "
        f"{slots}. Ask them to confirm which slot works best."
    )
